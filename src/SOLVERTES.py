    ...
    if direction == 'horizontal':
        for i in range(len(matrix[0])):
            if is_valid_move(row, i, direction):
                ...
                enumerate_combinations(row, i, 'vertical', buffer_size - 1, combination + [matrix[row][i]])
                used_coordinates.remove((row, i))
    elif direction == 'vertical':
        for i in range(len(matrix)):
            if is_valid_move(i, col, direction):
                used_coordinates.add((i, col))
                enumerate_combinations(i, col, 'horizontal', buffer_size - 1, combination + [matrix[i][col]])
...
for i in range(len(matrix[0])):
    ...
    enumerate_combinations(0, i, 'vertical', buffer_size - 1, [matrix[0][i]])
